        ...
        for t in range(max(processed_sequence.keys()) + 1):
            if t == 0:
                ...
                for candidate_s in candidate_ss:
                    ...
                    candidate_coordinates = compute_coordinates(candidate_s[0], dim_space=2)
...
def f_t_x(t, x: np.ndarray, processed_sequence: dict):
    if t == 0:
        ...
        while tau >= 0:
            if tau == 0:
                delta_x = x
                h_x_ = h_x(x=x, candidate_coordinates=compute_coordinates([], 2), tau=tau)
                nabla_g = np.dot((-processed_sequence[tau][0][1] / h_x_), delta_x.T).item()
            ...
            if tau not in processed_sequence.keys():
                ...
                for candidate_s in candidate_ss:
                    ...
                    delta_t = t - tau
                    # TODO: Change dim_space to dim space variable
                    candidate_coordinates = compute_coordinates(symbol_sequence=candidate_s[0], dim_space=2)
                    M_s = (1 - delta_t) * f_t_x(tau, candidate_coordinates, processed_sequence) + delta_t * m_s
                    delta_x = x - candidate_coordinates
                    h_x_ = h_x(x=x, candidate_coordinates=candidate_coordinates, tau=tau)
                    nabla_g = (f_t_x(tau, candidate_coordinates + h_x_, processed_sequence) - m_s) / h_x_
...
if __name__ == "__main__":
    ...
    print(f_t_x(7, np.array([1, 2]), processed_sequence=process_sequence(sequence=sequence_info)))
    # evaluate(
