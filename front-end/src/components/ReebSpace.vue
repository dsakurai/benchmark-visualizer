...
export default {
  ...
  created() {
    [this.innerWidth, this.innerHeight] = GraphUtils.marginConvention(
      this.figureInfo,
      this.margin
    );
  },
  ...
  methods: {
    ...
    plotSolverData(solverData) {
      let coordinatesData = GraphUtils.dataToCoordinates(
        solverData,
        this.xScale,
        this.yScale,
        this.rotate
      ...
      for (const [eval_node_id, coordinateData] of Object.entries(
        coordinatesData
      )) {
    ...
    getReebInfo() {
      if (!this.treeName || !this.dimension) {
        return;
      }
      axios
        .get(
          `/api/reeb_space?tree_name=${this.treeName}&dimension=${this.dimension}`
        )
        .then((response) => {
          ...
          this.svg = d3
            .select("#reebSpace")
            .append("svg")
            .attr("width", this.figureInfo.width)
            .attr("height", this.figureInfo.height);
          [this.xScale, this.yScale] = GraphUtils.composeScales(
            this.treeInfo,
    ...
    rotateGraph() {
      d3.selectAll("svg > *").remove();
      let sheetsCoordinates = GraphUtils.computeSheetsCoordinates(
        this.treeInfo,
        this.nodeInfo,
        this.rotate
      );
      ...
      this.drawAxis();
      let sheetsScale = GraphUtils.sheetCoordinatesToScale(
        sheetsCoordinates,
        this.xScale,
        this.yScale,
        this.logScale
