<template>
  <el-container>
    <el-main>
              ...
              <div class="tree-node">
                <el-space wrap :size="30">
                  ...
                  <span style="color: #ff4949"
                    >Best in selection: {{ nodeMinima[data.id].toFixed(2) }},
                    Best record:{{ locatedMinima[data.id].toFixed(2) }}</span
                  >
                  <!--                                <span-->
                ...
                <div class="progress-bar">
                  <el-progress
                    :stroke-width="15"
                    :percentage="
                      (nodeStats[data.id] * 100) / (stepRange * populationSize)
                    "
                    style="width: 600px"
                    ><span
                      style="width: 10px"
                      v-text="
                        (
                          (nodeStats[data.id] * 100) /
                          (stepRange * populationSize)
                        ).toFixed(2)
                      "
                    >
                    </span>
      ...
      <el-row>
        <ReebSpace
          :solver-data="filteredSolverData"
          :tree-name="treeType"
          :dimension="dimension"
          :all-ids="allIDs"
        ></ReebSpace>
...
export default {
  ...
  created() {
    this.get_available_exp_parameters();
    this.getNaiveLogData();
  },
  methods: {
    get_available_exp_parameters() {
      axios.get("/api/experiment_settings").then((response) => {
        response.data.solvers.forEach((solver) => {
          this.solverOptions.push({ value: solver, label: solver });
        });
        response.data.trees.forEach((tree) => {
          this.treeOptions.push({
            value: tree,
            label: this.snakeToTitleCase(tree),
          });
        });
        response.data.dimensions.forEach((dim) => {
          this.dimensionOptions.push({ value: dim, label: dim });
        });
        response.data.termination.forEach((termination) => {
          this.stoppingCriterionOptions.push({
            value: termination,
            label: termination,
          });
        });
    ...
    snakeToTitleCase(snakeCaseString) {
      // Return an empty string if the input is not a string or is empty
      if (!snakeCaseString || typeof snakeCaseString !== "string") {
        return "";
      }

      // 1. Split the string by underscores into an array of words
      return (
        snakeCaseString
          .split("_")
          // 2. Iterate over the array and capitalize each word
          .map((word) => {
    ...
    changeStepRange() {
      this.sliderStep[1] =
        this.sliderStep[0] + Math.round(this.stepRange * this.stepSize);
    },
    updateStep() {
      ...
      if (this.lockRange && this.sliderStep[1] < 100) {
        ...
        this.stepRange = Math.round(
          (this.sliderStep[1] - this.sliderStep[0]) / this.stepSize
        );
      }
      this.stepRange = Math.round(
        (this.sliderStep[1] - this.sliderStep[0]) / this.stepSize
      );
      let startStep = Math.round(
        (this.sliderStep[0] * this.populationSize) / this.stepSize
      );
      let endStep = Math.round(
        (this.sliderStep[1] * this.populationSize) / this.stepSize
      );
    ...
    getNaiveLogData() {
      ...
      axios
        .get(
          `/api/demo_data?solver=${this.solverType}&tree_name=${this.treeType}&dimension=${this.dimension}&termination=${this.stoppingCriterion}`
        )
        .then((response) => {
          ...
          this.stepMinimals = computeUtils.computeStepMinimal(
            this.solverData,
            this.allIDs
          );
    ...
    getStats(endStep) {
      console.log("Computing node counts @ nodeStats", endStep);
      console.log("Data", this.filteredSolverData);
      console.log(this.nodeMinima);
      console.log(this.locatedMinima);
      // this.locatedMinima = computeUtils.computeElapsedMinimal(this.elapsedData, this.allIDs);
      this.locatedMinima = computeUtils.getLocatedMinima(
        this.stepMinimals,
        endStep,
        this.allIDs
      );
      this.nodeMinima = computeUtils.computeElapsedMinimal(
        this.filteredSolverData,
        this.allIDs
      );
      this.nodeStats = computeUtils.computeNodeCounts(
        this.filteredSolverData,
        this.allIDs
      );
    },
