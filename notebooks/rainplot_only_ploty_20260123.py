...
plot_info = []
gen = 10
data_path = f"../utils/stats_output/gen_{gen}/"
        ...
        for file in files:
            if file.endswith(".csv"):
                try:
                    ...
                    dim = int(file[3]); obj = int(file[9])
                    tree = file[16:].strip(".csv")
                    df = pd.read_csv(src_file, index_col=0).fillna(0)
                    df["dimension"] = dim; df["n_objectives"] = obj; df["tree"] = tree
                    plot_info.append(df)
                except: continue
...
for solver in solvers:
    for tree in trees:
        filtered_df = new_df[(new_df["solver"] == solver) & (new_df["tree"] == tree)]
        if filtered_df.empty: continue

        # Reduced horizontal_spacing (0.05) to bring subplots closer
        fig = make_subplots(
            rows=1, cols=4, 
            subplot_titles=[f"N objectives = {obj}" for _, obj in dim_n_obj_combs],
        ...
        save_dir = "../utils/mass_combo/"
        os.makedirs(save_dir, exist_ok=True)
        fig.write_image(f"{save_dir}final_4_1_{solver}_{tree}.png", width=1600, height=400, scale=2)